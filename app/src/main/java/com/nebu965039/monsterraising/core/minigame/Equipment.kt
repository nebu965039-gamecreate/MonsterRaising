...
/** 装備アイテム 6 種のカタログ(8.4節)。装着できるのは常に 1 つ(8.4.1節)。 */
object Equipment {
    private val catalog: Map<ItemType, EquipEffect> = mapOf(
        ItemType.EQUIP_STRENGTH to EquipEffect(strengthGain = 0.05),
        ItemType.EQUIP_INTELLECT to EquipEffect(intellectGain = 0.05),
        ItemType.EQUIP_BALANCE_EFFECT to EquipEffect(intellectGain = 0.025, strengthGain = 0.025),
        ItemType.EQUIP_SATIETY to EquipEffect(satietyDecayCut = 0.05),
        ItemType.EQUIP_CLEANLINESS to EquipEffect(cleanlinessDecayCut = 0.05),
        ItemType.EQUIP_BALANCE_CARE to EquipEffect(satietyDecayCut = 0.025, cleanlinessDecayCut = 0.025),
    )
    ...
    fun effectOf(item: ItemType): EquipEffect? = catalog[item]
